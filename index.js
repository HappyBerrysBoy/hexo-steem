...
function updateSteemArticles(username) {
  steem.api.getDiscussionsByBlog({limit:10, tag:username}, function(err, result) {
    for (var i = 0; i < result.length; i++) {
      ...
      if (result[i].author == username){// || hexo.config.steem_resteems) {
        ...
        const excerpt = `<img src="images[0].replace(/\)/g, '')" />` + cont.substring(0, 200) + '.....';
...
// updateSteemArticles('happyberrysboy');